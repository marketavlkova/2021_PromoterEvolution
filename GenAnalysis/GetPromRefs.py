### Script to search for and extract intergenic / promoter regions
### from MG1655 genome according to a given RegulonDB TSS database
### example to run: python3 GetPromRefs.py <path to filtered RegulonDB csv file> <path to MG1655 genome file (GenBank)> [confidence level - optional]
...
def main():
    ...
    ### get annotations for all filtered TSS
            ...
            if isinstance(end2, int):
                for nt2 in range(end1, end2, 1):
                    ...
                    if any([isinstance(codon, int), isinstance(int_reg_end, int)]):
                        break
            ### find exact 5' position of intergenic region - considering '+ strand'
            if isinstance(end4, int):
    ...
    ### extract intergenic / promoter regions
